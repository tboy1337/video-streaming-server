...
class TestAuthenticationSecurity:
    ...
    def test_brute_force_protection_logging(self, test_server, test_config):
        ...
        failed_auth_attempts = [attempt for attempt in failed_attempts if not attempt[1]]
    ...
    def test_password_timing_attack_resistance(self, test_server, test_config):
        ...
        with test_server.app.test_request_context():
            ...
            time_ratio = max(valid_user_time, invalid_user_time) / min(valid_user_time, invalid_user_time)
...
class TestAuthorizationSecurity:
    ...
    def test_directory_traversal_authorization(self, authenticated_client, security_test_payloads):
        ...
        for payload in security_test_payloads['path_traversal']:
            ...
            assert response.status_code in [403, 404], f"Should block payload: {payload}"
            
            response = authenticated_client.get(f'/stream/{payload}')
            assert response.status_code in [403, 404], f"Should block stream payload: {payload}"


class TestInputValidationSecurity:
    """Test cases for input validation and sanitization"""
    
    def test_path_parameter_validation(self, authenticated_client, security_test_payloads):
    ...
    def test_query_parameter_validation(self, authenticated_client, security_test_payloads):
...
class TestInjectionAttackProtection:
    ...
    def test_xss_protection(self, authenticated_client, temp_video_dir, security_test_payloads):
        """Test XSS protection in file names and paths"""
        # Create files with XSS payloads in names
        for i, payload in enumerate(security_test_payloads['xss_payloads'][:3]):  # Limit to avoid timeout
            try:
    ...
    def test_sql_injection_protection(self, authenticated_client, security_test_payloads):
...
class TestSecurityLogging:
    ...
    def test_security_violation_metadata(self, test_server, tmp_path):
        ...
        security_logger.log_security_violation(
            'test_violation',
            'Test security violation details',
            '192.168.1.100'
        )
