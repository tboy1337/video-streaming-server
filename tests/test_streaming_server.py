...
class TestVideoStreamingServer:
    ...
    def test_security_configuration(self, test_server):
        ...
        assert app.config['PERMANENT_SESSION_LIFETIME'] == test_server.config.session_timeout
    ...
    def test_requires_auth_decorator_session_timeout(self, test_server):
        ...
        with test_server.app.test_request_context():
            with test_server.app.test_client() as client:
                with client.session_transaction() as sess:
                    sess['authenticated'] = True
                    sess['last_activity'] = time.time() - test_server.config.session_timeout - 1
...
class TestDirectoryListing:
    ...
    def test_directory_listing_without_auth(self, test_client):
        ...
        assert b'WWW-Authenticate' in response.headers.get('WWW-Authenticate', b'')
...
class TestVideoStreaming:
    ...
    def test_stream_path_traversal_attempt(self, authenticated_client, security_test_payloads):
        """Test streaming with path traversal attempts"""
        for payload in security_test_payloads['path_traversal'][:3]:  # Test first 3 to avoid timeout
...
class TestSessionManagement:
    ...
    def test_session_creation_on_auth(self, test_client, test_config):
    ...
    def test_session_persistence(self, test_client, test_config):
...
class TestRequestLogging:
    ...
    def test_security_event_logging(self, test_client, security_test_payloads):
        ...
        response = test_client.get(f'/stream/{security_test_payloads["path_traversal"][0]}')
